...
def tokenize(item, tokenizer, encoder_decoder=False):
    ...
    if tokenizer.name_or_path == f"{os.getenv('HOME')}/models/meta-llama/Llama-2-7b-chat-hf":
        shot = 1
        title = False
    elif tokenizer.name_or_path == f"{os.getenv('HOME')}/models/mistralai/Mistral-7B-Instruct-v0.3":
        shot = 3
        title = item['title']
    ...
    if not encoder_decoder:
        ...
        labels = tokenizer.encode(item['original_nq_answers'][:][0]['string'], add_special_tokens=True, return_tensors="pt")[0]
